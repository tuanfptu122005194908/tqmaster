...
import { TABLE_SCHEMAS, importFromZip, downloadTemplate, detectSheetsInFile, ImportResult } from '@/lib/excelBackup';
...
export default function BackupImportPanel() {
  ...
  const handleFileSelect = async (f: File) => {
    if (!f.name.endsWith('.zip') && !f.name.endsWith('.xlsx') && !f.name.endsWith('.xls')) {
      toast({ title: 'File không hợp lệ', description: 'Vui lòng chọn file ZIP hoặc Excel.', variant: 'destructive' });
      return;
    }
    setFile(f);
    setResult(null);
    try {
      ...
      toast({ title: 'Lỗi đọc file', description: 'Không thể đọc nội dung file.', variant: 'destructive' });
    }
  };
  ...
  const handleImport = async () => {
    ...
    setLoading(true);
    setProgress(0);
    setProgressMsg('Đang bắt đầu khôi phục...');
    setResult(null);

    try {
      // If it's a zip file, it will restore media. If it's an excel file, detectSheetsInFile handles it in JSZip?
      // Wait, importFromZip uses JSZip, so we must pass a zip file.
      // For simplicity, we assume users will upload the .zip generated by the new export.
      // If they upload a raw .xlsx, importFromZip might fail if it's not a zip archive.
      // Actually, an .xlsx is internally a zip archive, but it doesn't contain a 'database.xlsx' file inside it.
      // We should probably check if file is zip.
      // The instructions are to support the new ZIP approach.
      let res;
      if (file.name.endsWith('.zip')) {
        res = await importFromZip(file, selected, (p, msg) => {
          setProgress(p);
          setProgressMsg(msg);
        });
      } else {
        throw new Error('Tính năng Import Media chỉ hỗ trợ định dạng file .zip được xuất từ hệ thống mới.');
      }
      ...
      for (const tableName of selected) {
        queryClient.invalidateQueries({ queryKey: [tableName as never] });
      }
      queryClient.invalidateQueries({ queryKey: ['subjects'] as never });
      queryClient.invalidateQueries({ queryKey: ['exams'] as never });
      ...
      <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: '#10b981', marginTop: 2 }}>
                Khôi phục database & media từ file ZIP
              </div>
            </div>
          </div>
        </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, justifyContent: 'center' }}>
                ...
                <div style={{ textAlign: 'left' }}>
                  ...
                  <div style={{ fontSize: 11, color: '#6b7280' }}>
                    {detectedSheets.length} bảng được phát hiện: {detectedSheets.join(', ')}
                  </div>
                </div>
                <button
        ...
        {loading && (
          <div style={{ padding: '0 24px 16px', background: '#fff' }}>
            <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 6, display: 'flex', justifyContent: 'space-between', fontWeight: 500 }}>
              <span>{progressMsg || 'Đang khôi phục dữ liệu...'}</span>
