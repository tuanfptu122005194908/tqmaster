...
BEGIN
  ...
  VALUES (NEW.id, NEW.email, final_username, COALESCE(NEW.raw_user_meta_data->>'full_name', NEW.raw_user_meta_data->>'name', split_part(NEW.email, '@', 1)), NEW.raw_user_meta_data->>'student_code', NEW.raw_user_meta_data->>'phone')
...
BEGIN
  IF NEW.status = 'approved' AND OLD.status <> 'approved' THEN
    ...
    SELECT NEW.user_id, oi.subject_id, 'order' FROM public.order_items oi WHERE oi.order_id = NEW.id
    ON CONFLICT (user_id, subject_id) DO NOTHING;
  END IF;
  IF NEW.status = 'rejected' AND OLD.status = 'approved' THEN
    DELETE FROM public.user_subjects WHERE user_id = NEW.user_id AND subject_id IN (SELECT subject_id FROM public.order_items WHERE order_id = NEW.id) AND granted_by = 'order';
  END IF;
  RETURN NEW;
END;
$$;

CREATE TRIGGER trg_order_approved AFTER UPDATE ON public.orders FOR EACH ROW EXECUTE FUNCTION public.grant_subject_access_on_approve();
...
CREATE TRIGGER trg_update_conversation_last_message AFTER INSERT ON public.chat_messages FOR EACH ROW EXECUTE FUNCTION public.update_conversation_last_message();

CREATE OR REPLACE FUNCTION public.validate_order_amounts()
RETURNS trigger LANGUAGE plpgsql SET search_path = public AS $$
BEGIN
  IF NEW.original_amount IS NULL OR NEW.original_amount < 0 THEN RAISE EXCEPTION 'original_amount must be >= 0'; END IF;
  IF NEW.discount_amount IS NULL OR NEW.discount_amount < 0 THEN RAISE EXCEPTION 'discount_amount must be >= 0'; END IF;
  IF NEW.discount_amount > NEW.original_amount THEN RAISE EXCEPTION 'discount_amount cannot exceed original_amount'; END IF;
  IF NEW.final_amount IS NULL OR NEW.final_amount <= 0 THEN RAISE EXCEPTION 'final_amount must be > 0'; END IF;
  IF NEW.final_amount <> (NEW.original_amount - NEW.discount_amount) THEN RAISE EXCEPTION 'final_amount mismatch'; END IF;
  RETURN NEW;
END;
$$;

CREATE TRIGGER trg_validate_order_amounts BEFORE INSERT OR UPDATE OF original_amount, discount_amount, final_amount ON public.orders FOR EACH ROW EXECUTE FUNCTION public.validate_order_amounts();

CREATE OR REPLACE FUNCTION public.validate_order_item_price()
RETURNS trigger LANGUAGE plpgsql SET search_path = public AS $$
DECLARE db_price numeric;
BEGIN
  IF NEW.price IS NULL OR NEW.price < 0 THEN RAISE EXCEPTION 'order_items.price must be >= 0'; END IF;
  SELECT price INTO db_price FROM public.subjects WHERE id = NEW.subject_id;
  IF db_price IS NULL THEN RAISE EXCEPTION 'subject % not found', NEW.subject_id; END IF;
  IF NEW.price <> db_price THEN RAISE EXCEPTION 'price does not match'; END IF;
  RETURN NEW;
END;
$$;

CREATE TRIGGER trg_validate_order_item_price BEFORE INSERT OR UPDATE OF price, subject_id ON public.order_items FOR EACH ROW EXECUTE FUNCTION public.validate_order_item_price();
...
BEGIN
  DELETE FROM auth.users WHERE email_confirmed_at IS NULL AND created_at < now() - interval '10 minutes';
END;
$$;
...
CREATE POLICY "admins_update_all_profiles" ON public.profiles FOR UPDATE USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "users_view_own_roles" ON public.user_roles FOR SELECT USING (user_id = auth.uid());
...
CREATE POLICY "public_view_active_subjects" ON public.subjects FOR SELECT USING (is_active = TRUE OR public.has_role(auth.uid(), 'admin'));
CREATE POLICY "admins_manage_subjects" ON public.subjects FOR ALL USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "admins_manage_theory_subjects" ON public.theory_subjects FOR ALL USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "users_view_accessible_announcements" ON public.announcements FOR SELECT USING (subject_id IS NULL OR public.has_role(auth.uid(), 'admin') OR EXISTS (SELECT 1 FROM public.user_subjects us WHERE us.subject_id = announcements.subject_id AND us.user_id = auth.uid()));
CREATE POLICY "admins_manage_announcements" ON public.announcements FOR ALL USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "users_view_active_codes" ON public.discount_codes FOR SELECT USING (is_active = TRUE OR public.has_role(auth.uid(), 'admin'));
CREATE POLICY "admins_manage_codes" ON public.discount_codes FOR ALL USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));
GRANT SELECT, INSERT, UPDATE, DELETE ON public.discount_codes TO authenticated;
GRANT SELECT ON public.discount_codes TO anon;

CREATE POLICY "users_view_own_orders" ON public.orders FOR SELECT USING (user_id = auth.uid() OR public.has_role(auth.uid(), 'admin'));
CREATE POLICY "admins_update_orders" ON public.orders FOR UPDATE USING (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "admins_delete_orders" ON public.orders FOR DELETE USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "users_view_own_order_items" ON public.order_items FOR SELECT USING (EXISTS (SELECT 1 FROM public.orders WHERE id = order_items.order_id AND user_id = auth.uid()) OR public.has_role(auth.uid(), 'admin'));
CREATE POLICY "admins_manage_order_items" ON public.order_items FOR ALL USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "authenticated_read_settings" ON public.system_settings FOR SELECT TO authenticated USING (true);
CREATE POLICY "admins_manage_settings" ON public.system_settings FOR ALL USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "authenticated_view_news_posts" ON public.news_posts FOR SELECT TO authenticated USING (true);
CREATE POLICY "admins_manage_news_posts" ON public.news_posts FOR ALL USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "authenticated_view_news_likes" ON public.news_likes FOR SELECT TO authenticated USING (true);
CREATE POLICY "users_insert_own_like" ON public.news_likes FOR INSERT WITH CHECK (user_id = auth.uid());
CREATE POLICY "users_delete_own_like" ON public.news_likes FOR DELETE USING (user_id = auth.uid());

CREATE POLICY "authenticated_view_news_comments" ON public.news_comments FOR SELECT TO authenticated USING (true);
CREATE POLICY "users_insert_own_comment" ON public.news_comments FOR INSERT WITH CHECK (user_id = auth.uid());
CREATE POLICY "users_delete_own_comment" ON public.news_comments FOR DELETE USING (user_id = auth.uid());
CREATE POLICY "admins_delete_any_comment" ON public.news_comments FOR DELETE USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "conversations_select_own" ON public.conversations FOR SELECT USING (auth.uid() = user_id OR public.has_role(auth.uid(), 'admin'));
CREATE POLICY "conversations_insert_own" ON public.conversations FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "conversations_update_own" ON public.conversations FOR UPDATE USING (auth.uid() = user_id OR public.has_role(auth.uid(), 'admin'));
CREATE POLICY "conversations_delete" ON public.conversations FOR DELETE USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "chat_messages_select" ON public.chat_messages FOR SELECT USING (EXISTS (SELECT 1 FROM public.conversations c WHERE c.id = conversation_id AND (c.user_id = auth.uid() OR public.has_role(auth.uid(), 'admin'))));
CREATE POLICY "chat_messages_insert" ON public.chat_messages FOR INSERT WITH CHECK (auth.uid() = sender_id AND EXISTS (SELECT 1 FROM public.conversations c WHERE c.id = conversation_id AND (c.user_id = auth.uid() OR public.has_role(auth.uid(), 'admin'))));
CREATE POLICY "chat_messages_update" ON public.chat_messages FOR UPDATE USING (EXISTS (SELECT 1 FROM public.conversations c WHERE c.id = conversation_id AND (c.user_id = auth.uid() OR public.has_role(auth.uid(), 'admin'))));
CREATE POLICY "chat_messages_delete" ON public.chat_messages FOR DELETE USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "chat_cleanup_logs_select" ON public.chat_cleanup_logs FOR SELECT USING (auth.uid() = user_id OR public.has_role(auth.uid(), 'admin'));
CREATE POLICY "chat_cleanup_logs_insert" ON public.chat_cleanup_logs FOR INSERT WITH CHECK (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "chat_cleanup_logs_update" ON public.chat_cleanup_logs FOR UPDATE USING (auth.uid() = user_id OR public.has_role(auth.uid(), 'admin'));

CREATE POLICY "users_view_own_session" ON public.active_sessions FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "users_insert_own_session" ON public.active_sessions FOR INSERT WITH CHECK (user_id = auth.uid());
